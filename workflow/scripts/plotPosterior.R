...
p<-ggplot(thetas,aes(V1,V2))+geom_density_2d(aes(color=..level..))+geom_point(aes(x=test_param_x,y=test_param_y),colour='red',stroke=1,size=2,shape=4)+xlim(0,0.5)+ylim(0,0.5)+labs(x=expression(rho),y=expression(chi))+scale_color_continuous_sequential(palette="ag_Sunset")+theme(legend.position="none",text = element_text(size = 18))
print(p)
dev.off()
